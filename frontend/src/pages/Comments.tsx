    ...
    <div className="glass neon-border rounded-2xl p-5">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <h2 className="text-2xl font-semibold text-white">Twitter / Reddit feed</h2>
        </div>
          ...
          {["", "positive", "neutral", "negative"].map((f) => (
            <button
              ...
              className={`rounded-full px-3 py-2 text-xs uppercase tracking-wide ${
                filter === f ? "bg-cyan-500/20 text-cyan-100" : "border border-cyan-500/30 text-slate-200"
              }`}
            >
              {f === "" ? "All" : f}
            </button>
            ...
            <tbody>
              {data.map((p) => (
                <tr key={p.id} className="border-b border-slate-800/70">
                  ...
                  <td className="px-3 py-2 capitalize text-cyan-200">{p.sentiment_label || "pending"}</td>
