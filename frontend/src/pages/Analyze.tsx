    ...
    <div className="grid gap-4 md:grid-cols-[1.1fr_0.9fr]">
      <div className="glass neon-border rounded-2xl p-5">
        ...
        <div className="mt-5 flex items-center gap-3">
          <button onClick={submit} className="btn-primary rounded-xl px-5 py-3 text-sm" disabled={loading}>
          ...
          <div className="mt-3 space-y-3">
            <div className="rounded-xl border border-cyan-500/25 bg-slate-900/70 p-3">
              ...
              <div className="text-sm text-slate-300">
                {sentiment.total_posts} posts | {sentiment.negative_percentage.toFixed(1)}% negative
              </div>
            </div>
            <div className="rounded-xl border border-cyan-500/25 bg-slate-900/70 p-3">
              ...
              <div className="text-sm text-slate-300">
                Loss probability {(prediction.loss_probability * 100).toFixed(1)}% · Risk {prediction.risk_level}
              </div>
              <div className="mt-2 text-slate-300">{prediction.explanation}</div>
            </div>
            <button onClick={() => navigate("/dashboard")} className="btn-ghost w-full rounded-xl px-4 py-3 text-sm">
              Open Dashboard
            </button>
          </div>
        )}
        {!loading && !sentiment && (
          <div className="mt-2 text-sm text-slate-400">Run an analysis to see sentiment and risk results.</div>
        )}
      </div>
    </div>
  );
